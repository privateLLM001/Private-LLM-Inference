...
class Softmax(ClientModule):
  ...
  def forward(self, x):
    if self.replace_method == "none":
      ...
      ones = self.crypto.to_field(np.zeros_like(s)) # server has 1, client has 0
...
class SoftmaxSubstitute(ClientModule):
...
class ScaledDotProductAttention(ClientModule):
  ...
  def shares_matmul(self, helper, x_client, w_client, batchsize, input_dim, output_dim):
    def print_decrypt(prompt):
      print(prompt, self.crypto.matmul_decrypt_y(helper, self.crypto.matmul_deserialize_y(helper, self.recv())))
    
    x_client = x_client.copy()
    w_client = w_client.copy()
    
    # print("x_server =", self.recv()[0])
    # print("w_server =", self.recv()[0])
    # print("x_client =", x_client[0])
    # print("w_client =", w_client[0])
    ...
    # print("y_cc", y_cc)
    # print("y_ss", self.recv())

    # print_decrypt("y_cs + y_sc")
    # print("s =", self.recv()[0])
    # print_decrypt("y_cs + y_sc + y_s")
  ...
  def forward(self, qkv, attention_mask):
    ...
    for b in range(self.batchsize):
      ...
      # print("p_client", p_client)
      # debug(p_client)

      # don't divide by sqrt(E) here because input project has q's weights already divided
      p_client = self.comm.truncate(p_client)
      p_client = self.crypto.field_add(p_client, attention_mask[b])

      # self.comm.send(p_client)
...
class LayerNormSubstitute(ClientModule):
...
class TransformerEncoderStack(ClientModule):
...
class TakeCLSToken(ClientModule):
  def __init__(self, crypto, comm):
    super().__init__()
    self.crypto = crypto
    self.comm = comm
...
class BertWithoutEmbedding(ClientModule):

  def __init__(self, crypto, comm, stack, pooler, classifier):
    super().__init__()
    self.crypto = crypto
    self.comm = comm
    self.stack = stack
...
def client_model_from_description(des, crypto, comm):
  ...
  if t == "GELU":
    ...
    ret = Softmax(crypto, comm)
    ret.set_replace_method(des["replace_method"])
    return ret

  elif t == "ScaledDotProductAttention":
    ret = ScaledDotProductAttention(crypto, comm, des["dropout"])
    ret.softmax = client_model_from_description(des["softmax"], crypto, comm)
    return ret
  
  elif t == "MultiheadAttention":
    ret = MultiheadAttention(crypto, comm, 
      des["embed_dim"], des["num_heads"], des["dropout"])
    ret.attention = client_model_from_description(des["sda"], crypto, comm)
    return ret
  
  elif t == "LayerNorm":
    return LayerNorm(crypto, comm, des["dims"], des["eps"], des["affine"])
  
  elif t == "LayerNormSubstitute":
    return LayerNormSubstitute(crypto, comm, des["dims"])
    ...
    ret = TransformerEncoderLayer(
      crypto, comm,
      des["d_model"], des["num_heads"], 
      des["dim_feedforward"], des["activation"], des["dropout"]
    )
    ret.mha = client_model_from_description(des["mha"], crypto, comm)
    ret.norm1 = client_model_from_description(des["layernorm1"], crypto, comm)
    ret.norm2 = client_model_from_description(des["layernorm2"], crypto, comm)
    return ret
  
  elif t == "TransformerEncoderStack":
    ret = TransformerEncoderStack(
      crypto, comm,
      des["layer_count"],
      des["d_model"], des["num_heads"], 
      des["dim_feedforward"], des["activation"], des["dropout"]
    ...
    ret.layers = []
    for each in des["layers"]:
      ret.layers.append(client_model_from_description(each, crypto, comm))
    return ret
  ...
  elif t == "BertWithoutEmbedding":
    stack = client_model_from_description(des["stack"], crypto, comm)
    pooler = None if not des["has_pooler"] else client_model_from_description(des["pooler"], crypto, comm)
    classifier = client_model_from_description(des["classifier"], crypto, comm)
    return BertWithoutEmbedding(crypto, comm, stack, pooler, classifier)
  
  elif t == "Truncate":
    return Truncate(crypto, comm)
  
  elif t == "SoftmaxSubstitute":
