...
from crypto_gpu_cheetah import SCALE_BITS, PLAIN_BITS

if PLAIN_BITS == 59:
  from tools.cheetah_provider_alice import CheetahProvider
  from tools.sci_provider_alice import SCIProvider
elif PLAIN_BITS == 41:
  from tools.cheetah_provider_alice_41 import CheetahProvider
  from tools.sci_provider_alice_41 import SCIProvider
elif PLAIN_BITS == 37:
  from tools.cheetah_provider_alice_37 import CheetahProvider
  from tools.sci_provider_alice_37 import SCIProvider
else:
...
class ServerCommunication:
  ...
  def divide_element(self, x, d):
    shape = x.shape
    result = self.sci.div(x.flatten(), d.flatten())
    result = result.reshape(shape)
    return result
  ...
  def max(self, x, d):
    ...
    x = x.flatten()
    assert(len(x) % d == 0)
    result = self.cheetah.max(x.flatten(), len(x) // d)
    return result
