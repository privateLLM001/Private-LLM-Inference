...
class EvaluationUtils:
  ...
  def gelu(self, x):
    x = x.flatten()
    x_server_encoded = self.encoder.encode(x)


  def matmul_helper(self, batchsize, input_dims, output_dims):
    # print("MatmulHelper: ", batchsize, input_dims, output_dims)
    ret = pytroy.MatmulHelper(batchsize, input_dims, output_dims, self.slot_count)
    # print("Helper ok")
    return ret
  ...
  def matmul_serialize_encoded_weights(self, helper, weights):
    return helper.serialize_encoded_weights(weights)
  
  def matmul_deserialize_encoded_weights(self, helper, s):
    return helper.deserialize_encoded_weights(s)
  ...
  def encode_flattened(self, x):
    n = len(x)
    li = 0
    ret = []
    while li < n:
      ui = li + self.slot_count
      if ui > n: ui = n
      plain = self.encoder.encode(x[li:ui])
      ret.append(plain)
    return ret
  
  def encrypt_flattened(self, x):
    ret = []
    for each in x:
      ret.append(self.encryptor.encrypt(each))
    return ret
